...
Adhere to Go defaults: tabs for indentation, `go fmt ./...` before pushing, and idiomatic lower-case package names. Favor descriptive but concise exported names (e.g., `DaemonManager`, `ProxySession`). Run `golangci-lint run` when available; the Makefile exposes it via `make lint`. Keep configuration structs and JSON/YAML tags in sync with files in `docs/` and `test_configs/`. When adding CLI flags, mirror naming patterns already in `internal/cli`.

## Testing Guidelines
Stage unit tests alongside implementation packages using `_test.go` suffixes. Prefer table-driven cases and embed behavior summaries using “Given/When/Then” comments for clarity. Place longer fixtures under `test_configs/`. Validate concurrent code with `go test -race ./...`. For new daemon or proxy flows, include smoke tests that exercise both direct and daemon-backed execution paths. If a bug survives two iterations, capture it with a failing test before attempting another fix.
