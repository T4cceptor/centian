...
func buildScorecardArtifactRecord(scorecard *RunScorecard) (*persistence.BenchmarkArtifactRecord, error) {
	if scorecard == nil {
		return nil, fmt.Errorf("run scorecard is required")
	}
	sessionPath := filepath.Clean(strings.TrimSpace(scorecard.SessionPath))
	runPath := filepath.Clean(strings.TrimSpace(filepath.Dir(scorecard.RunManifestPath)))
	payload, err := json.Marshal(scorecard)
	if err != nil {
		return nil, fmt.Errorf("marshal run scorecard: %w", err)
	}
	attempt := scorecard.Attempt
	return &persistence.BenchmarkArtifactRecord{
		ID:                 benchmarkArtifactID(string(persistence.BenchmarkArtifactKindScorecard), scorecard.SuiteID, benchmarkSessionID(scorecard.SuiteID, sessionPath), sessionPath, runPath),
		ArtifactKind:       persistence.BenchmarkArtifactKindScorecard,
		SuiteID:            scorecard.SuiteID,
		SessionID:          benchmarkSessionID(scorecard.SuiteID, sessionPath),
		SessionPath:        sessionPath,
		RunPath:            runPath,
		...
		CreatedAtUnixMilli: scorecard.GeneratedAt.UnixMilli(),
		PayloadJSON:        payload,
	}, nil
}

func buildSummaryArtifactRecord(summary *SessionSummary) (*persistence.BenchmarkArtifactRecord, error) {
	if summary == nil {
		return nil, fmt.Errorf("session summary is required")
	}
	sessionPath := filepath.Clean(strings.TrimSpace(summary.SessionPath))
	payload, err := json.Marshal(summary)
	if err != nil {
		return nil, fmt.Errorf("marshal session summary: %w", err)
	}
	return &persistence.BenchmarkArtifactRecord{
		ID:                 benchmarkArtifactID(string(persistence.BenchmarkArtifactKindSummary), summary.SuiteID, benchmarkSessionID(summary.SuiteID, sessionPath), sessionPath, ""),
		ArtifactKind:       persistence.BenchmarkArtifactKindSummary,
		SuiteID:            summary.SuiteID,
		SessionID:          benchmarkSessionID(summary.SuiteID, sessionPath),
		SessionPath:        sessionPath,
		ScoreVersion:       summary.ScoreVersion,
		CreatedAtUnixMilli: summary.GeneratedAt.UnixMilli(),
		PayloadJSON:        payload,
	}, nil
}

func buildComparisonArtifactRecord(comparison *ComparisonSummary) (*persistence.BenchmarkArtifactRecord, error) {
	if comparison == nil {
		return nil, fmt.Errorf("comparison summary is required")
	}
	sessionPath := filepath.Clean(strings.TrimSpace(filepath.Join(comparison.RootPath, comparison.SuiteID)))
	sessionID := benchmarkSessionID(comparison.SuiteID, sessionPath)
	payload, err := json.Marshal(comparison)
	if err != nil {
		return nil, fmt.Errorf("marshal comparison summary: %w", err)
	}
	return &persistence.BenchmarkArtifactRecord{
		ID:                 benchmarkArtifactID(string(persistence.BenchmarkArtifactKindComparison), comparison.SuiteID, sessionID, sessionPath, ""),
		ArtifactKind:       persistence.BenchmarkArtifactKindComparison,
		SuiteID:            comparison.SuiteID,
		SessionID:          sessionID,
		SessionPath:        sessionPath,
		ScoreVersion:       comparison.ScoreVersion,
		CreatedAtUnixMilli: comparison.GeneratedAt.UnixMilli(),
		PayloadJSON:        payload,
	}, nil
}
